class Jump extends Phaser.Scene {
    ...
    create() {
        ...
        // add some physics clouds (with collision!)
        ...
        // add collision tiles for the clouds
        this.clouds = this.add.group();
        for(let i = 530; i < 530 + tileSize * 4; i += tileSize) {
            let cloudTile = this.physics.add.sprite(i, 100, 'platformer_atlas', 'cloud_tile01').setScale(SCALE).setOrigin(0);
            cloudTile.body.immovable = true;
            cloudTile.body.setAllowGravity(false).setVelocityX(25);
            this.clouds.add(cloudTile);
        }
        for(let i = 130; i < 130 + tileSize * 4; i += tileSize) {
            let cloudTile = this.physics.add.sprite(i, 200, 'platformer_atlas', 'cloud_tile02').setScale(SCALE).setOrigin(0);
            cloudTile.body.immovable = true;
            cloudTile.body.setAllowGravity(false).setVelocityX(45);
            this.clouds.add(cloudTile);
        }
    ...
    update() {
        ...
        this.physics.world.wrap(this.clouds, this.cloud01.width/2);
    }
}